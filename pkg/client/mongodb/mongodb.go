...
// создаём клиента для подключения к дб
func NewClient(ctx context.Context, host, port, username, password, database, authDB string) (db *mongo.Database, err error) {
	...
	if err != nil {
		return nil, fmt.Errorf("faild to client to mongoDB due to error %v", err)
	}
	//пингуем дб
	if err = client.Ping(ctx, nil); err != nil {
		return nil, fmt.Errorf("faild to ping to mongoDB due to error %v", err)
	}
